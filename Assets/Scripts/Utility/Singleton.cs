...
namespace Kudoshi.Utilities
{
    ...
    public class Singleton<T> : MonoBehaviour where T : MonoBehaviour
    {
        ...
        public static void SetSingletonDontDestroyOnLoad(T singleton)
        {
            ...
            if (singleton != Singleton<T>.Instance)
            {
                Debug.Log($"{singleton.name} singleton instance already exist! Destroying the new one");
                UnityEngine.Object.Destroy(singleton.gameObject);
                return;
            }

            Util.SetDontDestroyOnLoad(Singleton<T>.Instance.gameObject);
        }
    }
}
